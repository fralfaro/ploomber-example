...
def cs_sidebar():
    ...
    st.sidebar.markdown('''
<small>The [RMS Titanic](https://es.wikipedia.org/wiki/RMS_Titanic) was a British passenger
 ...
 from Southampton, England, to New York City. It
  ...
  rg and went down, resulting in a significant loss of life.</small>
    ...
    surrounding the tragic sinking of the RMS Titanic by harnessing the power of machine learning and a user-friendly web application framework called Streamlit. </small> ''', unsafe_allow_html=True)
...
def cs_body():
    ...
    def load_data():
        ...
        data = pd.read_csv(
            'https://raw.githubusercontent.com/fralfaro/ploomber-example/main/data/train.csv')  # Replace 'titanic.csv' with your own data file
    ...
    num_rows = col2.slider("Number of rows to display", min_value=1, max_value=50, value=5)
    ...
    plot_type_options = ['univariate', 'bivariate']  # You can add more plot types if you wish
    ...
    if plot_type == 'univariate':
        ...
        fig.update_traces(marker=dict(line=dict(color='black', width=1)), marker_color='skyblue')
        st.plotly_chart(fig)
    else:
        # Create a grouped histogram plot with Plotly
        fig = px.histogram(df, x=x_column, color='Survived', title=f'Histogram of {x_column} by Survived')
        # Display the plot in Streamlit
    ...
    col1, col2, col3, col4 = st.columns(4)
    x_column_options_cat = ['Pclass', 'SibSp', 'Parch', 'Sex', 'Cabin', 'Embarked']  # You can add more columns if you wish
    x_column_cat = col1.selectbox('Select column:', options=x_column_options_cat)
    plot_type_options_cat = ['univariate', 'bivariate']  # You can add more plot types if you wish
    ...
    if plot_type_cat == 'univariate':
        ...
        # Create a grouped histogram plot with Plotly
        fig = px.histogram(df, x=x_column_cat, color='Survived', title=f'Barplot of {x_column_cat} by Survived')
        # Display the plot in Streamlit
        fig.update_traces(marker=dict(line=dict(color='black', width=1)))
        st.plotly_chart(fig)
    ...
    def load_metrics_data():
        data = pd.read_csv(
            'data/metrics.csv')  # Replace 'titanic.csv' with your own data file
    ...
    def load_predictions_data():
        data = pd.read_csv(
            'data/y_predictions.csv')  # Replace 'titanic.csv' with your own data file
    ...
    sorted_df = metrics.sort_values(by=selected_column, ascending=False).reset_index(drop=True)
    st.write(sorted_df)
    ...
    def load_importances_data():
        data = pd.read_csv(
            'data/feature_importances.csv')  # Replace 'titanic.csv' with your own data file

        return data

    df_importances = load_importances_data()

    fig3 = px.bar(df_importances.loc[lambda x: x['importances'] > 0].sort_values(by='importances'), x='importances', y='feature_names',
                 orientation='h')
    fig3.update_layout(title='Feature Importances',
                      xaxis_title='Feature',
    ...
    def load_test_data():
        ...
        data = pd.read_csv(
            'data/predictions.csv')  # Replace 'titanic.csv' with your own data file
